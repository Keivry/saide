...
/// Events generated by the mapping editor UI
#[derive(Debug, Clone)]
...
impl MappingEditor {
    ...
    pub fn draw(&mut self, ctx: &egui::Context, params: EditorParams) -> EditorEvent {
        ...
        ctx.input(|input| {
            ...
            {
                event = EditorEvent::AddMapping(click_pos);
            }
            ...
            if input.pointer.secondary_clicked()
                ...
                && params.video_rect.contains(click_pos)
            {
                event = EditorEvent::DeleteMapping(click_pos);
            }
        });
