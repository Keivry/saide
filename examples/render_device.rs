...
fn main() -> Result<()> {
    ...
    info!("Starting real device renderer...");
    ...
    eframe::run_native(
        "Saide Renderer",
        native_options,
        Box::new(|cc| Ok(Box::new(DeviceRendererApp::new(cc, serial)))),
    )
    .map_err(|e| anyhow::anyhow!("eframe error: {}", e))
}

struct DeviceRendererApp {
...
impl eframe::App for DeviceRendererApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("top_panel").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ui.heading("📱 Real Device Renderer");
                ui.separator();
