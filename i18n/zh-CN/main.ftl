...
probe-codec-dialog-message = 未找到设备 { $serial } 的编解码器配置，需要运行一次检测以确定最优编码参数。
probe-codec-dialog-message-stale = 设备 { $serial } 的编解码器配置已过期，需要重新检测以获取最新最优配置。
probe-codec-dialog-hint = ⚠ 检测期间请保持设备屏幕亮起并解锁，否则可能导致检测失败。
