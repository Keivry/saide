...
public final class Server {
    ...
    private static void internalMain(String... args) throws Exception {
        ...
        // PATCH: anti-detection - masquerade process name and silence scrcpy log output
        try {
            java.lang.reflect.Method setArgV0 = android.os.Process.class.getDeclaredMethod("setArgV0", String.class);
            setArgV0.setAccessible(true);
            setArgV0.invoke(null, "com.android.systemui:recents");
        } catch (Exception e) {
            // silently ignore
        }

        try {
            java.lang.reflect.Field tagToLevel = android.util.Log.class.getDeclaredField("sLogLevels");
            tagToLevel.setAccessible(true);
            @SuppressWarnings("unchecked")
            java.util.Map<String, Integer> levels = (java.util.Map<String, Integer>) tagToLevel.get(null);
            if (levels != null) {
                levels.put("scrcpy", android.util.Log.ASSERT);
            }
        } catch (Exception e) {
            // silently ignore
        }
