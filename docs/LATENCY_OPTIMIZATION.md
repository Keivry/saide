...
### 第 2-3 周: 阶段二优化 (零拷贝 GPU) - ❌ **已终止**

> **最终状态 2026-01-15**: wgpu v28 验证失败,不支持外部内存导入,**Phase 2 正式终止**。  
> **详细调研**: `docs/PHASE2_ZEROCOPY_FEASIBILITY.md` (850+ 行技术分析 + v28 验证结果)

**原目标**: 实现零拷贝 GPU 解码 (VAAPI/NVDEC → Vulkan 外部内存)

- [x] **技术调研完成** - 已确认技术可行性
- [x] **wgpu v27 原型测试** - hal API 不可用
- [x] **wgpu v28 兼容性验证** - 仍无外部内存 API
- [ ] ~~VAAPI DMA-BUF 导出~~ (终止)
- [ ] ~~Vulkan 外部内存导入~~ (终止)

**终止原因**:
1. wgpu v28 **仍未暴露** `Device::as_hal()` 或外部内存 API
2. 切换到 ash (raw Vulkan) 需重写渲染管线 (2-3 周) + 放弃跨平台支持
3. **Phase 1 已达目标** (30-50ms → 20-35ms) + **Phase 3 可再降 8-18ms**
4. **投入产出比低**: 2-3 周工作仅换 12-20ms 延迟降低

**技术遗产**:
- ✅ 完整可行性分析文档 (供未来 wgpu v29+ 重新评估)
- ✅ 原型测试代码 (`examples/test_vulkan_import.rs`)
- ✅ 已向 wgpu 项目提 Feature Request (长期跟踪)

**下一步**: 转向 **Phase 3** (音频 + 输入优化,实施成本低且收益稳定)

---

### 第 4 周: 阶段三优化 (音频 + 输入) - **建议优先**
...
|---------|---------|---------|-----------|--------|
| 消除解码器缓冲 | 5-10ms | 低 | 低 | P0 ⭐⭐⭐⭐⭐ | ✅ Phase 1 完成 |
| 零拷贝 GPU 解码 | 10-15ms | 高 | 中 | P1 ⭐⭐⭐⭐ | ❌ 暂缓 (wgpu限制) |
| TCP_QUICKACK | 3-5ms | 低 | 低 | P0 ⭐⭐⭐⭐⭐ | ✅ Phase 1 完成 |
| 移除过度 flush | 2-5ms | 低 | 低 | P0 ⭐⭐⭐⭐⭐ | ✅ Phase 1 完成 |
| CPAL 独占模式 | 3-8ms | 中 | 中 | P1 ⭐⭐⭐⭐ | ⏭️  Phase 3 建议 |
| 原始输入设备 | 5-10ms | 高 | 中 | P2 ⭐⭐⭐ | ⏭️  Phase 3 可选 |
| UDP 视频流 | 10-20ms | 极高 | 高 | P4 ⭐⭐ | 研究阶段 |
| 禁用 AV 同步 | 5-10ms | 低 | 高 | P3 ⭐⭐ | 配置化待定 |
